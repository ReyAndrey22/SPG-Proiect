...
public:
    ...
    float GetHeight(float x, float z) const {
        ...
        float j = (x / xzScale) + (mapWidth / 2.0f);
        float i = (z / xzScale) + (mapHeight / 2.0f);

        int cellX = static_cast<int>(floor(j));
        int cellZ = static_cast<int>(floor(i));

        if (cellX < 0 || cellX >= mapWidth || cellZ < 0 || cellZ >= mapHeight) {
            return -10.0f; 
        }

        unsigned char pixelValue = heightData[cellZ * mapWidth + cellX];
        return (float)pixelValue * yScale - 10.0f;
    }

    void Draw(Shader const& shader, glm::mat4 const& view, glm::mat4 const& projection) const {
        shader.use();
